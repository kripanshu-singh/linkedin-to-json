...
window.LinkedinToResumeJson = (() => {
    ...
    /** @param {SchemaVersion} version */
    LinkedinToResumeJson.prototype.parseAndShowOutput = async function parseAndShowOutput(version = 'stable', email = '', color = '', job = '') {
        const rawJson = await this.parseAndGetRawJson(version);
        ...
        this.sendToN8nWebhook({ ...rawJson, image, color, job, basics: { ...rawJson.basics, email: newEmail } });

        const parsedExport = {
            raw: { ...rawJson, image, email, color, job },
